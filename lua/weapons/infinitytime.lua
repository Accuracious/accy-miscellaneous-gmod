...
SWEP.PrintName				= "Infinity Bad Time"
			...
			if( ent:IsPlayer() && ent:Health() > 0 ) then
				if( ent:GetActiveWeapon():IsValid() && ent:GetVar( "miss", NULL ) != NULL ) then
					if( CurTime() >= ent:GetVar( "miss", NULL ) ) then
						if( ent:GetActiveWeapon():GetClass() == "infinitytime" ) then
							net.Start( "miss" )
								net.WriteEntity( ent )
...
function SWEP:Deploy( )
	...
	if( self.BeatSound ) then
		...
		self.BeatSound = CreateSound( self.Owner, Sound( "undertale/infinity.mp3" ) )
		self.BeatSound:Play()
	end
...
function SWEP:Think()
	...
	if( self.Owner:KeyPressed( IN_RELOAD ) && self:GetVar( "blockBoneGround" ) == 0 ) then
		self:SetVar( "blockBoneGround", CurTime() + 0.01 )
		...
		for i = 1, 25 do
			...
			if( traceGrd.Hit ) then
				local ent = ents.Create( "ent_infinityground" )
				ent:SetAngles( ang + Angle( 45, 0, 0 ) )
				ent:SetVar( "pos", pos )
		...
		if( grabbedEnt:IsValid() ) then
			...
			if( self:GetVar( "invincible" ) == 0 ) then
				...
				if( tr.HitWorld && grabbedEnt:GetVelocity():Length() > 200 ) then
					...
					grabbedEnt:TakeDamage( grabbedEnt:GetVelocity():Length() / 0.01, self.Owner, self.Owner )
					
					grabbedEnt:EmitSound( Sound( "undertale/sans/smash.wav" ), 75, 100, 1, CHAN_AUTO ) 
					//EmitSound( Sound( sound ),grabbedEnt:GetPos() )
				end
			elseif ( CurTime() - self:GetVar( "invincible" ) ) >= 0.0 then
				self:SetVar( "invincible", 0 )
			end
		end
	end
...
function SWEP:PrimaryAttack()
	...
	//if( !self:GetVar( "grabbedEnt", NULL ):IsValid() ) then
		...
		local ent = ents.Create( "ent_infinityblaster" )
...
function SWEP:SecondaryAttack()
	...
	if( !entGrabbed:IsValid() ) then
		...
		local ent = ents.Create( "ent_infinitythrow" )
		local side
		...
		phys:SetVelocity( ent:GetUp() * 101001 )
	else
