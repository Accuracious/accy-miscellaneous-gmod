...
function SWEP:Deploy( )
	...
	if( self.BeatSound ) then
		...
		self.BeatSound = CreateSound( self.Owner, Sound( "undertale/stmpwyfs.mp3" ) )
		self.BeatSound:Play()
	end
	...
	if( self.Owner:KeyPressed( IN_RELOAD ) && self:GetVar( "blockBoneGround" ) == 0 ) then
		...
		for i = 1, 15 do
			...
			if( traceGrd.Hit ) then
				local ent = ents.Create( "agencyground" )
...
function SWEP:PrimaryAttack()
	...
	//if( !self:GetVar( "grabbedEnt", NULL ):IsValid() ) then
		...
		local ent = ents.Create( "agencyblaster" )
...
function SWEP:SecondaryAttack()
	...
	if( !entGrabbed:IsValid() ) then
		...
		local ent = ents.Create( "agencythrow" )
		local side
