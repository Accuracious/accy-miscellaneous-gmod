...
function ENT:Initialize()
	if( SERVER ) then
		self:SetModel( "models/theforgottenarchitect/props/bullets/308winchester.mdl" )
...
function ENT:PhysicsUpdate()
	if( SERVER ) then
		...
		if( value > 10 ) then
			if( value < 15 ) && !self:GetVar( "reverse", NULL ) && !sound1 then
				...
				util.TraceHull( {
					...
					filter = function( ent )
						if( ent:IsValid() ) then
						//if( ent:GetClass() == "ent_infinitythrow") then return false end
		...
		local val1 = math.sin( ( math.max( 0, value ) + 50 ) / 10 ) * 150
		local val2 = ( seed - 0.5 ) * 13
		
		self:SetPos( position + self:GetUp() * ( val1 - val2 - 100 ) )
