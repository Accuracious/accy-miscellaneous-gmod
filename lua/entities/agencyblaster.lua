...
function ENT:Initialize()
	if( SERVER ) then
		local model = "models/dronesrewrite/helldrone/helldrone.mdl"
...
function ENT:Draw()
	...
	if( self:GetVar( "shootEffect", NULL ) > 0 ) then
		...
		render.SetColorMaterial( 0, 255, 0 )
		...
		for i = 1, 5 do
			...
			render.DrawBeam( pos + dir * ( forw - ( length / 5 ) ), pos + dir * forw, ( 15 + 3 * i ) * size, 1, 1, Color( 0, 255, 0, 255 ) ) 
		end
		
		render.DrawBeam( pos + dir * ( 16 + length ), tr.HitPos, 30 * size, 1, 1, Color( 0, 255, 0, 255 ) )
	end
end